...
class SFTDatasetGenerator(BaseDatasetGenerator):
    """
    SFT（监督微调）数据集生成器
    
    支持两种模式：
    1. 完整模式：生成instruction、input、output
    2. 固定指令模式：使用固定instruction，生成input、output
    """
    ...
    def generate_input_output_sample(self, instruction: Optional[str] = None) -> Dict[str, str]:
        """
        为给定的instruction生成input和output（固定指令模式）
